...
namespace GizaTraffic.Services
{
    public class DBService<T> : IBaseServiceInterface<T> where T : BaseModel
    {
        ...
        {
            _uow = uow;
            _rep = new Repository<T>(_uow?.DBContext);
        }
        ...
        public async Task<(IEnumerable<T> Items, int TotalCount)>? Get(Expression<Func<T, bool>> predicate,  Func<T, object> sort, int skip, int take, bool ascending)
        {
            if (_rep == null)
                return (Enumerable.Empty<T>(), 0);
            return await _rep.Get(predicate, sort, skip, take);
        }
        ...
        public Task<(IEnumerable<T> Items, int TotalCount)>? Get(Func<T, object> sort, int skip, int take, bool ascending)
        {
